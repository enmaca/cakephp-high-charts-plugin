...
class DataTableComponent extends Component {
    ...
    public function beforeRender(Controller $controller) {
        ...
        $this->Controller->set('dtColAlign',  $this->_colalign);
    ...
    public function process() {
      if (isset($this->Controller->paginate[$this->_Model->alias])) {
            $this->query = Set::merge($this->Controller->paginate[$this->_Model->alias], $this->query);
        }
        $total = $this->_Model->find('count', $this->query+array('contain' => false));
        $this->_sort();
        $this->_search();
        $totalDisplayed = $this->_Model->find('count', $this->query+array('contain' => false));
        $this->_paginate();
        if( isset($this->settings['findType']) ) {
            $findType = $this->settings['findType'];
        } else {
            $findType = 'all';
        }
        $results = $this->_Model->find($findType, $this->query);
        $dataTableData = array(
            'iTotalRecords' => $total,
        ...
        $this->Controller->set('PaginatorSettings', $this->settings);
    ...
    public function isDataTableRequest() {
        ...
        if (is_string($trigger) && is_callable(array(
            $this->Controller,
            $trigger
        ))) {
    ...
    protected function _parseSettings() {
         if( isset($this->settings['colalign']) ){
            $this->_colalign = $this->settings['colalign'];
        }
        ...
        if( isset($this->settings['group']))
            $this->query['group'] = $this->settings['group'];
    }
    ...
    protected function _isDataTableRequest() {
        if ($this->request->params['pass'][(count($this->request->params['pass'])-1)] == 'datatable' ) {
            return true;
        }
        return false;
    }
    ...
    protected function _paginate() {
        if ($this->_params['iDisplayLength']>0 && isset($this->_params['iDisplayStart'])) {
        ...
        if(!empty($this->settings['conditions'])){
            $this->query['conditions'] = $this->settings['conditions'];
        }
    }
        ...
        foreach ($this->_columns as $column => $options) {
            if ($options!==null) {
                ...
                if ($searchable!==false) {
                    ...
                    if (is_string($searchable) && is_callable(array(
                        $this->_Model,
                        $searchable
                    ))) {
    ...
    protected function _getModel() {
        ...
        if( isset($this->settings['PaginateModel']) ){
            if( isset( $this->Controller->{$this->settings['PaginateModel']}) ){
                return $this->Controller->{$this->settings['PaginateModel']};
            }
        }
